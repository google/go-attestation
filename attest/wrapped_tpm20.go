...
// challenge should be generated by GenerateEkChallenge.
func (k *wrappedKey20) certifyWithDecryptionEk(tb tpmBase, ek *EK, challenge DecryptionEkChallenge) (*CertificationParameters, error) {
	t, ok := tb.(*wrappedTPM20)
	if !ok {
		return nil, fmt.Errorf("expected *wrappedTPM20, got %T", tb)
	}
...
func ekAuthSession(rwc io.ReadWriter) (tpmutil.Handle, tpm2.AuthCommand, error) {
	...
	auth := tpm2.AuthCommand{Session: sessHandle, Attributes: tpm2.AttrContinueSession}
	return sessHandle, auth, nil
}
