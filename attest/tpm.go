...
type tpmBase interface {
	...
	loadAppKey(opaqueBlob []byte) (*ApplicationKey, error)
	newAppKey(ak *AK, opts *AppKeyConfig) (*ApplicationKey, error)
...
// NewAppKey creates an application key certified by the attestation key ak.
func (t *TPM) NewAppKey(ak *AK, opts *AppKeyConfig) (*ApplicationKey, error) {
	return t.tpm.newAppKey(ak, opts)
}

// LoadAppKey loads a previously-created application key into the TPM for use.
// A key loaded via this function needs to be closed with .Close().
// Only blobs generated by calling AppKey.Serialize() are valid parameters
// to this function.
func (t *TPM) LoadAppKey(opaqueBlob []byte) (*ApplicationKey, error) {
	return t.tpm.loadAppKey(opaqueBlob)
}
