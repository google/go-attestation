...
type tpmBase interface {
	...
	loadSK(opaqueBlob []byte) (*SK, crypto.Signer, error)
	newSK(ak *AK, opts *SKConfig) (*SK, crypto.Signer, error)
...
// NewSK creates a signing key certified by the attestation key ak. It returns
// the created key and its signer.
func (t *TPM) NewSK(ak *AK, opts *SKConfig) (*SK, crypto.Signer, error) {
	return t.tpm.newSK(ak, opts)
}

// LoadSK loads a previously-created SK into the TPM for use. It returns
// the key as well as its Signer.
// A key loaded via this function needs to be closed with .Close().
// Only blobs generated by calling SK.Serialize() are valid parameters
// to this function.
func (t *TPM) LoadSK(opaqueBlob []byte) (*SK, crypto.Signer, error) {
	return t.tpm.loadSK(opaqueBlob)
}
